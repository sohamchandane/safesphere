...
export const WeatherDisplay = ({ location, embedded = false }: WeatherDisplayProps) => {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        toast({
          title: t('weatherWidget.unavailableTitle', { defaultValue: 'Weather data unavailable' }),
          description: t('weatherWidget.unavailableDescription', { defaultValue: 'Could not fetch weather data' }),
          variant: 'destructive',
        });
      } finally {
        setLoading(false);
  ...
  const getSeverityStyle = (value: number, safeValue: number, higherIsWorse = true): SeverityTone => {
    ...
    if (ratio <= 1) return { label: t('weatherWidget.levelSafe', { defaultValue: 'Safe' }), className: 'text-success', badgeClassName: 'bg-success/10 text-success border-success/20' };
    if (ratio <= 1.5) return { label: t('weatherWidget.levelWatch', { defaultValue: 'Watch' }), className: 'text-warning', badgeClassName: 'bg-warning/10 text-warning border-warning/20' };
    return { label: t('weatherWidget.levelHigh', { defaultValue: 'High' }), className: 'text-destructive', badgeClassName: 'bg-destructive/10 text-destructive border-destructive/20' };
  };
  ...
  const getAQICategory = (aqi: number): SeverityTone => {
    if (aqi <= 50) return { label: t('weatherWidget.aqiGood', { defaultValue: 'Good' }), className: 'text-success', badgeClassName: 'bg-success/10 text-success border-success/20' };
    if (aqi <= 100) return { label: t('weatherWidget.aqiModerate', { defaultValue: 'Moderate' }), className: 'text-warning', badgeClassName: 'bg-warning/10 text-warning border-warning/20' };
    if (aqi <= 150) return { label: t('weatherWidget.aqiSensitive', { defaultValue: 'Unhealthy for Sensitive Groups' }), className: 'text-warning', badgeClassName: 'bg-warning/10 text-warning border-warning/20' };
    if (aqi <= 200) return { label: t('weatherWidget.aqiUnhealthy', { defaultValue: 'Unhealthy' }), className: 'text-destructive', badgeClassName: 'bg-destructive/10 text-destructive border-destructive/20' };
    if (aqi <= 300) return { label: t('weatherWidget.aqiVeryUnhealthy', { defaultValue: 'Very Unhealthy' }), className: 'text-destructive', badgeClassName: 'bg-destructive/10 text-destructive border-destructive/20' };
    return { label: t('weatherWidget.aqiHazardous', { defaultValue: 'Hazardous' }), className: 'text-destructive', badgeClassName: 'bg-destructive/10 text-destructive border-destructive/20' };
  };
    ...
    return [
      {
        ...
        numeric: weather.aqi,
        safe: 100,
        unit: t('weatherWidget.airQualityIndex', { defaultValue: 'Air Quality Index' }),
        icon: Cloud,
        higherIsWorse: true,
      ...
      {
        label: t('weatherWidget.temperature', { defaultValue: 'Temperature' }),
        value: `${weather.temperature}°C`,
        numeric: weather.temperature,
        safe: 26,
        unit: t('weatherWidget.safeTempRange', { defaultValue: 'Safe: 18-26°C' }),
        icon: Thermometer,
        higherIsWorse: true,
        description: t('weatherWidget.comfortZone', { defaultValue: 'Comfort zone for most users' }),
      },
      {
        label: t('weatherWidget.pressure', { defaultValue: 'Pressure' }),
        value: `${weather.pressure} hPa`,
        numeric: weather.pressure,
        safe: 1013,
        unit: t('weatherWidget.safePressure', { defaultValue: 'Safe: ~1013 hPa' }),
        icon: Wind,
        higherIsWorse: false,
        description: t('weatherWidget.stablePressure', { defaultValue: 'Stable atmospheric pressure reference' }),
      },
      {
        label: 'PM2.5',
        ...
        icon: Droplets,
        higherIsWorse: true,
        description: t('weatherWidget.pm25Description', { defaultValue: 'Fine particulate matter' }),
      },
      {
        label: 'PM10',
        ...
        icon: Droplets,
        higherIsWorse: true,
        description: t('weatherWidget.pm10Description', { defaultValue: 'Larger inhalable particles' }),
      },
      {
        label: 'O3',
        ...
        icon: Cloud,
        higherIsWorse: true,
        description: t('weatherWidget.o3Description', { defaultValue: 'Ground-level ozone' }),
      },
      {
        label: 'NO2',
        ...
        icon: Cloud,
        higherIsWorse: true,
        description: t('weatherWidget.no2Description', { defaultValue: 'Traffic-related pollutant' }),
      },
      {
        label: 'SO2',
        ...
        icon: Cloud,
        higherIsWorse: true,
        description: t('weatherWidget.so2Description', { defaultValue: 'Combustion-related pollutant' }),
      },
      {
        label: 'CO',
        ...
        icon: Cloud,
        higherIsWorse: true,
        description: t('weatherWidget.coDescription', { defaultValue: 'Carbon monoxide concentration' }),
      },
    ];
  }, [weather, t]);
  ...
  if (loading) {
    return embedded ? (
      <div className="rounded-2xl border border-border/70 bg-background/75 p-4 text-sm text-muted-foreground" style={panelStyle}>
        {t('weatherWidget.loading', { defaultValue: 'Loading weather data...' })}
      </div>
    ) : (
      <Card className="border-0 shadow-soft">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('weatherWidget.title', { defaultValue: 'Weather Data' })}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">{t('weatherWidget.loading', { defaultValue: 'Loading weather data...' })}</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="border-0 shadow-soft content-visibility-auto">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Cloud className="h-5 w-5 text-primary" />
          {t('weatherWidget.heading', { defaultValue: 'Weather & Air Quality' })}
        </CardTitle>
        <CardDescription>{t('weatherWidget.currentConditions', { defaultValue: 'Current environmental conditions' })}</CardDescription>
      </CardHeader>
        ...
        <div className="space-y-4 rounded-2xl border border-border/70 bg-background/70 p-4" style={panelStyle}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{t('weatherWidget.currentConditions', { defaultValue: 'Current environmental conditions' })}</p>
            </div>
            <div className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
              {t('weatherWidget.severityIndicators', { defaultValue: 'Severity-coded indicators' })}
            </div>
          </div>
          {renderTiles()}
        </div>
